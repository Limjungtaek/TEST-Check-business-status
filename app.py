...
st.set_page_config(
    page_title="사업자 등록 상태 조회 시스템 v2.8",
    page_icon="🏢", 
    layout="wide"
)

# [개선] 모든 환경에서 글씨가 잘 보이도록 CSS 전면 수정
st.markdown("""
    <style>
    /* 전체 배경색 흰색 고정 */
    .main {
    ...
    /* 이용 방법 안내 박스 - 배경 연한 파랑, 글씨 진한 검정 */
    .guide-box {
        background-color: #E0F2FE !important; /* 연한 하늘색 */
        padding: 25px;
        border-radius: 12px;
        border-left: 6px solid #0284C7;
        margin-bottom: 30px;
        color: #1E293B !important; /* 진한 남검색 (가독성 핵심) */
    }
    .guide-box h3 {
    ...
    [data-testid="stMetricLabel"] p {
        color: #BAE6FD !important; /* 아주 연한 하늘색 */
...
<div class="guide-box">
    ...
    <ul>
        ...
        <li><b>엑셀 데이터:</b> 엑셀 파일은 무조건 <b>첫 번째 열(A열)</b>의 데이터를 인식합니다.</li>
        <li><b>추출 기준:</b> 숫자로만 구성된 10자리 이상의 번호만 정상적으로 추출됩니다.</li>
    </ul>
</div>
""", unsafe_allow_html=True)
...
with guide_col1:
    ...
    if os.path.exists(txt_img_path):
        ...
        st.image(resized_txt, caption="메모장 작성 예시 (한 줄에 하나씩)", width=500)
    else:
...
with guide_col2:
    ...
    if os.path.exists(excel_img_path):
        ...
        st.image(resized_excel, caption="엑셀 작성 예시 (A열에 입력)", width=500)
    else:
...
if uploaded_file:
    ...
    try:
        ...
        if biz_nums:
            ...
            st.warning("유효한 사업자 번호를 찾을 수 없습니다. 형식을 확인해 주세요.")
