...
st.set_page_config(
    page_title="사업자 등록 상태 조회 시스템 v2.3",
    page_icon="🏢", 
    layout="wide"
)
...
with st.expander("📌 이용 방법 및 파일 형식 예시 (확인하려면 클릭)", expanded=False):
    ...
    with guide_col1:
        ...
        if os.path.exists(txt_img_path):
            ...
            # [핵심] 이미지 크기를 250x250 픽셀로 강제 조정
            # PIL 버전에 따라 Image.Resampling.LANCZOS 대신 Image.ANTIALIAS 사용 가능
            resized_txt = img_txt.resize((250, 250), Image.Resampling.LANCZOS)
            st.image(resized_txt, caption="TXT 파일 작성 예시", width=250)
        else:
    ...
    with guide_col2:
        ...
        if os.path.exists(excel_img_path):
            ...
            # [핵심] 이미지 크기를 250x250 픽셀로 강제 조정
            resized_excel = img_excel.resize((250, 250), Image.Resampling.LANCZOS)
            st.image(resized_excel, caption="Excel 파일 작성 예시", width=250)
        else:
