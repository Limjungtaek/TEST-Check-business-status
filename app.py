...
st.set_page_config(
    page_title="사업자 등록 상태 조회 시스템 v2.6",
    page_icon="🏢", 
    layout="wide"
)

# [수정] 메트릭(조회 결과 카드) 시인성 강화를 위한 CSS
st.markdown("""
    <style>
    /* 전체 배경색 */
    .main {
    ...
    /* [핵심] 메트릭 카드 디자인 - 배경을 어둡게, 글씨를 밝게 고정 */
    [data-testid="stMetric"] {
        background-color: #1E3A8A !important; /* 짙은 남색 배경 */
    ...
    /* 메트릭 변화량(Delta) 글자색 (필요 시) */
    [data-testid="stMetricDelta"] > div {
        color: #FFD700 !important; /* 노란색 계열로 포인트 */
    }
    </style>
...
with st.expander("📌 이용 방법 및 파일 형식 예시 (확인하려면 클릭)", expanded=False):
    st.markdown("파일 형식에 상관없이 **하이픈(`-`)과 모든 공백**은 시스템이 자동으로 제거합니다.")
    ...
    with guide_col1:
        ...
        if os.path.exists(txt_img_path):
            ...
            st.image(resized_txt, caption="TXT 파일 작성 예시", width=500)
        else:
            st.info("💡 `images/txt_example.png` 이미지를 준비해 주세요.")
    ...
    with guide_col2:
        ...
        if os.path.exists(excel_img_path):
            ...
            st.image(resized_excel, caption="Excel 파일 작성 예시", width=500)
        else:
            st.info("💡 `images/excel_example.png` 이미지를 준비해 주세요.")
...
if biz_nums and st.button(f"🚀 {len(biz_nums)}건 국세청 실시간 조회 시작"):
    ...
    # [수정된 부분] CSS가 적용되어 이제 흰색 배경에 묻히지 않습니다.
